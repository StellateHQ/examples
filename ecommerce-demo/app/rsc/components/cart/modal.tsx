...
function CartItem({ item, i, closeCart }: { item: CartItem; i: number; closeCart: () => void }) {
  const [message, setMessage] = useState<string | undefined>();
  return (
    <li className="flex w-full flex-col border-b border-neutral-300 dark:border-neutral-700">
      <div className="relative flex w-full flex-row justify-between px-1 py-4">
        <div className="absolute z-40 -mt-2 ml-[55px]">
          <DeleteItemButton productId={item.id} i={i} />
        </div>
        <Link
          id={`cart-item-${i}`}
          href={`/rsc/product/${item.slug}`}
          onClick={closeCart}
...
export default function CartModal({ cart }: { cart: CartData | undefined }) {
                ...
                <div className="flex h-full flex-col justify-between overflow-hidden p-1">
                  <ul className="flex-grow overflow-auto py-4">
                    {cart.items.map((item, i) => (
                      <CartItem key={item.slug} item={item} i={i} closeCart={closeCart} />
                    ))}
                  </ul>
