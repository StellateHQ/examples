...
pub type Variables<'a> = Map<'a, &'a str, Value<'a>>;
...
pub struct ObjectValue<'a> {
    pub children: List<'a, ObjectField<'a>>,
}
...
impl<'a> ObjectValue<'a> {
    ...
    pub fn as_map(&'a self, ctx: &'a ASTContext) -> Map<'a, &'a str, &'a Value<'a>> {
        let map = Map::new();
        for field in self {
            map.insert(&ctx.arena, field.name, &field.value);
        }
        map
    }
}
...
pub struct Arguments<'a> {
    pub children: List<'a, Argument<'a>>,
}
...
impl<'a> Arguments<'a> {
    ...
    pub fn as_map(&'a self, ctx: &'a ASTContext) -> Map<'a, &'a str, &'a Value<'a>> {
        ...
        for argument in self {
            map.insert(&ctx.arena, argument.name, &argument.value);
        }
        map
    }
}
...
pub struct Directives<'a> {
    pub children: List<'a, Directive<'a>>,
}
...
pub struct SelectionSet<'a> {
    pub selections: List<'a, Selection<'a>>,
}
...
pub struct Field<'a> {
    ...
    /// Field's `name`.
    ///
    /// When a Field has no alias this property is set to the same reference or value as `name`.
...
impl<'a> Field<'a> {
    ...
    /// defaults.
    #[inline]
    pub fn new_leaf(name: &'a str) -> Self {
        Field {
            alias: None,
            name,
    ...
    /// defaults.
    #[inline]
    pub fn new_aliased_leaf(alias: &'a str, name: &'a str) -> Self {
        Field {
...
impl<'a> Selection<'a> {
    ...
    #[inline]
    pub fn directives(&'a self) -> &'a Directives<'a> {
        match self {
    ...
    /// Resolves @include and @skip directives to a flag on whether the current selection must be
    /// skipped during execution.
    ///
    /// [Reference](https://spec.graphql.org/October2021/#sec--skip)
    #[inline]
    pub fn should_include(&'a self, variables: Option<&Variables<'a>>) -> bool {
...
pub struct VariableDefinitions<'a> {
    pub children: List<'a, VariableDefinition<'a>>,
}
...
impl<'a> VariableDefinitions<'a> {
    ...
    pub fn as_map(&'a self, ctx: &'a ASTContext) -> Map<'a, &'a str, &'a VariableDefinition<'a>> {
        let map = Map::new();
        for var_def in self {
            map.insert(&ctx.arena, var_def.variable.name, var_def);
        }
        map
    }
}
...
#[derive(Debug, PartialEq, Clone, Copy)]
pub struct FragmentDefinitionWithIndex<'a> {
...
pub struct Document<'a> {
    pub definitions: List<'a, Definition<'a>>,
...
impl<'a, 'b> Document<'a> {
    ...
    /// their definitions.
    pub fn fragments_with_index(
        &'a self,
        ctx: &'a ASTContext,
    ) -> Map<'a, &'a str, FragmentDefinitionWithIndex<'a>> {
        let map = Map::new();
        for (index, definition) in self.into_iter().enumerate() {
    ...
    pub fn fragments(
        &'a self,
        ctx: &'a ASTContext,
    ) -> Map<'a, &'a str, &'a FragmentDefinition<'a>> {
        let map = Map::new();
        for definition in self {
            if let Definition::Fragment(fragment) = definition {
                map.insert(&ctx.arena, fragment.name.name, fragment);
            }
        }
        map
    }

    fn operation_and_index(
        &'a self,
        ...
        if let Some(by_name) = by_name {
            self.into_iter()
                ...
                .find_map(|(index, definition)| {
                    definition
                        .operation()
                        .and_then(|operation| match operation.name {
                            Some(NamedType { name }) if name == by_name => Some((operation, index)),
                            _ => None,
                        })
                })
                .ok_or(Error::new(
                    format!("Operation {by_name} does not exist"),
            ...
            if operations.len() == 1 {
                ...
                Err(Error::new(
                    "Document does not contain exactly one operation",
                    Some(ErrorType::GraphQL),
                ))
            }
        }
    ...
    /// Finds an operation definition by name or the single operation in the document when `None`
    /// is passed.
    ///
    ...
    pub fn operation(&'a self, by_name: Option<&'b str>) -> Result<&'a OperationDefinition<'a>> {
        Ok(self.operation_and_index(by_name)?.0)
    }

    /// Finds the index of an operation definition inside the list of definitions of the given
    /// [`Document`] by name or the single operation in the document when `None` is passed.
    ///
    /// [Reference](https://spec.graphql.org/October2021/#GetOperation())
    pub fn operation_index(&'a self, by_name: Option<&'b str>) -> Result<usize> {
        Ok(self.operation_and_index(by_name)?.1)
    }
}
