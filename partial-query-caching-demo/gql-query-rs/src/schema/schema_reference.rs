...
impl<'a> SchemaReference<'a> {
    ...
    #[inline]
    pub fn from_object_type(object: &'a SchemaObject<'a>) -> Self {
        SchemaReference {
    ...
    pub fn select_field(&mut self, field_name: &'a str) -> Result<OutputType<'a>> {
        ...
        if let Some(field) = fields.get(field_name) {
            let output_type = *field.output_type.of_type();
        ...
        match self.pointer {
            ...
            OutputType::Object(schema_object) => {
                let maybe_interface = schema_object
                    .get_interfaces()
                    .into_iter()
                    .find(|interface| interface.name == type_name)
                    .map(|interface| OutputType::Interface(interface));
            ...
            OutputType::Interface(interface) => {
                let maybe_interface = interface
                    .get_interfaces()
                    .into_iter()
                    .chain(interface.get_possible_interfaces().into_iter())
                    .find(|interface| interface.name == type_name)
                    .map(|interface| OutputType::Interface(interface));
                if let Some(output_type) = maybe_interface {
                    self.push_pointer(output_type);
                ...
                let maybe_object = interface
                    ...
                    .map(|object| OutputType::Object(object));
            ...
            OutputType::Union(schema_union) => {
                let maybe_object = schema_union
                    .get_possible_type(type_name)
                    .map(|object| OutputType::Object(object));
                if let Some(output_type) = maybe_object {
                    self.push_pointer(output_type);
                    Ok(output_type)
