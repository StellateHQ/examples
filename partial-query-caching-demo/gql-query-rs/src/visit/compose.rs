...
impl<'a, C, A: Visitor<'a, C>, B: Visitor<'a, C>> ComposedVisitor<'a, C, A, B> {
    ...
    fn compose_flow_enter<'b, Node: 'a>(
        &mut self,
        fn_a: fn(_self: &mut A, ctx: &C, node: &'a Node, info: &VisitInfo) -> VisitFlow,
        fn_b: fn(_self: &mut B, ctx: &C, node: &'a Node, info: &VisitInfo) -> VisitFlow,
        node: &'a Node,
        info: &VisitInfo,
        ctx: &C,
    ) -> VisitFlow {
...
impl<'a, C, A: Visitor<'a, C>, B: Visitor<'a, C>> Visitor<'a, C> for ComposedVisitor<'a, C, A, B> {
    ...
    #[inline]
    fn enter_field(&mut self, ctx: &C, field: &'a Field<'a>, info: &VisitInfo) -> VisitFlow {
        self.compose_flow_enter(A::enter_field, B::enter_field, field, info, ctx)
    }

    #[inline]
    fn leave_field(&mut self, ctx: &C, field: &'a Field<'a>, info: &VisitInfo) -> VisitFlow {
...
mod tests {
    ...
    impl<'a> Visitor<'a, ()> for SkipVisitor {
        ...
        fn enter_field(&mut self, _: &(), _field: &Field, _info: &VisitInfo) -> VisitFlow {
            self.enter_field += 1;
            VisitFlow::Skip
        }
        fn leave_field(&mut self, _: &(), _field: &Field, _info: &VisitInfo) -> VisitFlow {
            self.leave_field += 1;
            VisitFlow::Skip
        }

        fn enter_directive(&mut self, _: &(), _field: &Directive, _info: &VisitInfo) -> VisitFlow {
            self.enter_directive += 1;
            VisitFlow::Skip
        }
        fn leave_directive(&mut self, _: &(), _field: &Directive, _info: &VisitInfo) -> VisitFlow {
