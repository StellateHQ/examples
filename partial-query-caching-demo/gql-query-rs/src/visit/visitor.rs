...
pub(crate) mod private {
    ...
    pub trait VisitNodeWithInfo<'a>: Sized + Copy {
...
pub trait Visitor<'a, Context = ()>: Sized + 'a {
...
pub trait VisitNode<'a>: Sized + Copy + private::VisitNodeWithInfo<'a> {
...
pub(crate) mod tests {
    ...
    impl<'a> Visitor<'a, ()> for CountVisitor {
        ...
        fn leave_document(&mut self, _: &(), _document: &Document, _info: &VisitInfo) -> VisitFlow {
            self.out_document += 1;
            VisitFlow::Next
        }
        ...
        fn enter_argument(&mut self, _: &(), _argument: &Argument, _info: &VisitInfo) -> VisitFlow {
            self.in_argument += 1;
            VisitFlow::Next
        }
        fn leave_argument(&mut self, _: &(), _argument: &Argument, _info: &VisitInfo) -> VisitFlow {
            self.out_argument += 1;
            VisitFlow::Next
        }
    }
